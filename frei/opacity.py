...
def mapfunc_exact(group, temperature=2500, pressure=1e-08, interp_kwargs=interp_kwargs):
    ...
    wl = group.wavelength
    op = group.interp(temperature=temperature, pressure=pressure, **interp_kwargs).opacity
    
    Delta_x = wl.max() - wl.min()
    result = xr.apply_ufunc(
        np.trapz, op, wl, dask='allowed', #dask='parallelized',
        input_core_dims=[["temperature", "pressure", "wavelength"], ["wavelength"]],
...
def delayed_map_exact_concat(grouped, temperatures, pressures, lam, client):
    ...
    for i, (name, group) in enumerate(grouped):
        results.append(dask.delayed(mapfunc_exact)(group, temperature=temperatures.value, pressure=pressures.to(u.bar).value))
    # results = dask.delayed(grouped.map)(mapfunc_exact, temperature=temperatures.value, pressure=pressures.to(u.bar).value)
    ...
    ).interp(dict(wavelength=lam.to(u.um).value), method='linear', kwargs=dict(fill_value='extrapolate'))
...
def binned_opacity(
    ...
    for i, path in enumerate(paths): 
        ...
        print(f'Loading opacity for {species_name} ({i+1}/{len(paths)}): group in wavelength')
        species_grouped = species_ds.groupby_bins("wavelength", wl_bins)
        print(f'Loading opacity for {species_name} ({i+1}/{len(paths)}): compute exact k-distributions')
        species_binned = delayed_map_exact_concat(species_grouped, temperatures, pressures, lam, client)
...
# Malik 2017 Eqn 17
n_lambda_H2 = lambda wavelength: 13.58e-5 * (1 + (7.52e-11 * u.cm**2) * wavelength**-2) + 1
# Deitrick 2020 Eqn C3
n_lambda_He = lambda wavelength: 1e-8 * (2283 + (1.8102e13 / (1.5342e10 - (wavelength / (1 * u.um))**-2))) + 1
    ...
    for species in opacities: 
        ops.append(
            fastchem_mmr[species] * opacities[species].interp(
                dict(temperature=temperature.value, pressure=pressure.to(u.bar).value),
